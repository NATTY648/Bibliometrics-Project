...
def draw_network(G: nx.Graph, out_path: str | Path, title: str = '', top_labels: int = 12, seed: int = 42,
    ...
    """Draw a readable network map with optional cluster coloring and overlapping avoidance.
    ...
    pos = nx.spring_layout(G, seed=seed, weight='weight', k=1.5/np.sqrt(G.number_of_nodes()))
    weighted_degree = dict(G.degree(weight='weight'))
    node_sizes = [100 + 40 * weighted_degree.get(n, 1) for n in G.nodes()]
    ...
    nx.draw_networkx_edges(G, pos, width=edge_widths, alpha=0.20, edge_color='#bdc3c7', ax=ax)
    nx.draw_networkx_nodes(G, pos, node_size=node_sizes, alpha=0.88, node_color=node_colors, edgecolors='#2c3e50', linewidths=0.6, ax=ax)
    
    top_nodes = sorted(G.nodes(), key=lambda n: weighted_degree.get(n, 0), reverse=True)[:top_labels]
    
    # Advanced label placement using adjustText to avoid overlaps
    texts = []
    for n in top_nodes:
        x, y = pos[n]
        # Shorten label for cleaner view
        label = n[:25] + '...' if len(n) > 25 else n
        texts.append(plt.text(x, y, label, fontsize=8.5, fontweight='semibold',
                              ha='center', va='center', color='#2c3e50',
                              bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#7f8c8d", lw=0.5, alpha=0.9)))
                              
    try:
        from adjustText import adjust_text
        node_x = [pos[node][0] for node in G.nodes()]
        node_y = [pos[node][1] for node in G.nodes()]
        adjust_text(texts, x=node_x, y=node_y, force_static=(0.2, 0.4), force_text=(0.2, 0.4),
                    expand=(1.2, 1.3), arrowprops=dict(arrowstyle="-", color='#7f8c8d', lw=0.6, alpha=0.7))
    except Exception as e:
        # Fallback to standard labels if adjustText fails
        print(f"Warning: adjustText failed, falling back to standard labels: {e}")
        labels = {n: n[:25] for n in top_nodes}
    ...
    if show_clusters:
        ...
        if legend_handles:
            plt.legend(handles=legend_handles, loc='upper left', fontsize=8, framealpha=0.90, title='Communities', title_fontsize=9)
